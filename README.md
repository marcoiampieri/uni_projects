# uni_projects

### This repository contains some of the projects I worked on during my university years, from both my Bachelor's and Master's studies.

## Structure

- **bachelor_projects/**
  * exam_project
  * game_of_life
  * root_lab_2nd_year
  * lab_3rd_year
    - experiment_1_diodes
    - experiment_2_bjt_transistors
    
 
- **master_projects/**
  * bologna
    - cshep_exam_project
    - prep_for_research
    - root_module
  * clermont-ferrand
    - ML_course
    - statistics_course
  * dortmund_projects
    - IceCube_data_analysis
    - Scintillating_Fibers_analysis
    - SiliconStrip_analysis 
  * thesis

## Notes
Some of the files and pieces of code (especially those created during the years of my Bachelor's studies) are written in Italian.  

As of March $31^{st}$ 2026, I'm still in the process of sorting and uploading my projects, mainly:
- [x] Update the bachelor's labs' README files 
- [x] Add some of the data and outputs for the bachelor's labs
- [x] Update the clermont-ferrand/ folder
- [x] Update the README file for the master_project/ folder
- [x] Update the README files for the clermont-ferrand/ folder
- [x] Add the Bologna semester root folder
- [x] Update the Bologna semester root folder
- [x] Update the Bologna semester root folder README file
- [ ] Add some of the data for the Dortmund labs

